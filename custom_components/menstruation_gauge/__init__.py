...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    @callback
    def _refresh_cycle_model_at_scheduled_time(now: datetime) -> None:
        """Notify cycle sensors from the Home Assistant event loop."""
        async_dispatcher_send(hass, SIGNAL_HISTORY_UPDATED)

    runtime.unregister_midnight_listener = async_track_time_change(
        hass,
        _refresh_cycle_model_at_scheduled_time,
        hour=5,
        minute=0,
        second=0,
    )
